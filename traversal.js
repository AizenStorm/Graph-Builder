...
function dfs(start)	{
	start -=1;
    var it_count=num_nodes;
	console.log("start dfs");
	traversalData += "The order for Depth-First Traversal  starting from NODE : " + (start+1) + " <br>";
	while(it_count>0) {
		if(!graphNodes[start].visited)	{
			explore(start);
		}
...
function bfs(start)	{
	...
	traversalData += "The order for Breadth-First Traversal starting from NODE : " + (start+1) + " <br> ";
	queue.push(start);
